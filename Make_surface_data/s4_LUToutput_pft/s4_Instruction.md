...
之间唯一的差别就是格子尺度。0.5° 是 42 × 27 = 594 个植被格点，1 km 是 134 万个。

优势 PFT 占比随分辨率的变化（2023，含并列，一律判给低索引）：

| PFT | 0.01° | 0.5° | 变化 |
|---|---|---|---|
| 1 针叶常绿 | 34.6% | **42.6%** | **+8.0** |
| 7 阔叶落叶 | 30.1% | 31.1% | +1.0 |
| 13 C3 草 | 20.7% | 15.5% | −5.2 |
| 15 crop | 13.8% | 10.3% | −3.5 |
| 9 灌木 | 0.3% | **0.0%** | 完全消失 |
| 0 Bare_Ground | 0.5% | 0.5% | — |

典型的 winner-take-all：格子一粗，分布最广的类型把少数类型吞掉。针叶常绿净增 8
个百分点，草和作物各让出 5.2 / 3.5，灌木在 56 km 的格子里一格都赢不到。密西西比
河谷的作物带因为足够连片还能留下，佐治亚/卡罗来纳那种细碎的作物镶嵌则整个消失。

另外粗网格上只要格内有一点植被就算植被格点，所以 0.5° 的海岸线是块状的，域看起来
比 1 km 的还"胖"一圈。
...
2. `fig2_dominant_pft_*` — 优势 ELM 自然 PFT，每个 PFT 单独一个图例项。
   ...
   **9↔10 不算歧义**：两者按构造恒等，判给 9 就是正确答案，不是抛硬币，所以
   stdout 里单列一行、不混进"歧义并列"。**PFT 9 与 PFT 10 按构造恒等**，所以 10 在图上永远赢不了；它仍被强制留在图例里
